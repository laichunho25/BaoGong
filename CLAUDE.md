# CLAUDE.md — 包公 BaoGong 專案憲法

> 這份文件每次對話都會被載入。保持精簡；細節放 `docs/`。

## 1. 專案是什麼

**包公 BaoGong** — 香港 TCSP（Trust or Company Service Provider）持牌秘書公司的
**中立比較 + 真實評價 + 報價撮合**平台。
主要用戶：不熟悉香港開公司／開戶流程的**內地客戶**。

名字取兩義：包拯的「公道」，以及「包羅香港『公』司服務」。借的是公正，不是官威——
平台不是官方、不替任何人裁決。完整品牌規則見 `docs/BRAND.md`，寫文案前必讀。

三個核心價值，各對應包公的一個樣子：
1. **明鏡高懸／透明**：只列公司註冊處官方持牌名單，每日同步，價格區間公開。
2. **鐵面無私／可信**：評價須以 NNC1 文件核驗「確有合作」為前提，才給「已驗證」標記。
3. **一把尺量到底／撮合**：客戶發 RFQ，秘書公司主動報價（已認領免費公司每月 3 個，訂閱 15／40，每張需求最多 8 家報價）。

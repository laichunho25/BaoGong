...
- **首頁「精選用家評語」只放已發布且經 NNC1 核驗的評價**，排序用時間不用分數
  ...
  沒有合格評價時寧可顯示空狀態。
- **徵求評價可以給回報，但回報不得與評價內容掛鈎。** 三條同時成立才准上線：
  (1) 回報只看「有沒有經文件核驗」，不看打幾分——給一星與給五星拿到的東西完全一樣；
  (2) 回報是什麼、怎麼拿到，必須寫在**開口要評價的那一頁**上，不能只寫在條款裡；
  (3) **平台不以金錢或等值折扣購買評價**，頁面上要明說。
  已落實：首頁「邀請已開公司的股東」區塊。回報只有兩樣，而且兩樣都真的存在——
  `reviews.selectors.is_verified_reviewer`（衍生值，不落欄位：評價被隱藏或核驗被推翻，
  資格同時消失）以及需求牆上的軟性優先排序（`rfq.selectors.open_rfqs` 的 `buyer_verified`）。
  排序是**軟性**的：沒有標記的需求單照樣在牆上，只是排後面，而且這件事寫在那一頁上，
  不是暗地裡的加權。行銷文案最容易出現的違規是承諾一個 code 裡不存在的好處，
  所以 `apps/core/tests/test_home.py::TestReviewInvitation` 拿頁面上的每項承諾去對查詢驗證。
- 需求牆上的「已核實用家」標記是**需求單的屬性，不是人的身分**：標記只說「這位買家背後有一份
  被核驗過的文件」，不說他是誰（見 §4）。

...
- `[P5] 報價額度仍是每日制，PRD §3.7 已改為每月制` — `RFQ_FREE_QUOTES_PER_DAY = 3` 與 `QuotaLedger` 的每日流水帳要換成每月週期（免費 5／基本 20／進階 60），且報價前提要多一道「仍在官方名單上」以外的層級判斷 — P7-0 之後、P7 訂閱落地之前處理，換算邏輯只改 `rfq.services`，`QuotaLedger` 的流水帳形狀不變。

...
- ~~`[P7-0] 掃毒器預設是 UnavailableScanner，所以本機永遠傳不出一個能發佈的 logo`~~ — 已改成
  說出來而不是靜靜壞掉：`scanning_available()` 為假時 `upload_logo()` 直接拒收（拒絕放在 service
  層，因為拿掉模板上的表單擋不住記得 URL 的人），管理頁改印「標志上傳功能正在準備中」。
  首次部署就是這個狀態，開啟步驟見 `docs/DEPLOY_RENDER.md` §3.1。
- `[P7-0] 首次部署不開掃毒器，所以 logo 功能上線即關閉` — 本體是 Render 沒有託管 ClamAV，
  自架 pserv 要 2GB RAM 的方案 — 認領證明也一起卡著（NNC1 傳得上去但審不了），
  所以這不只是 logo 的事；預算定了就照 §3.1 加 pserv，P8 checklist 要驗一次真的掃得動。

...
- `[P5] 報價額度有兩套互斥的制度` — 現行 `RFQ_FREE_QUOTES_PER_DAY = 3` 是**每日**額度、發給所有已認領公司；PRD §3.7 的 5／20 是**訂閱週期**額度、只發給付費層 — 兩者同時存在的話，免費公司每月拿 90 單、付費公司拿 5 單，付費就變成懲罰。P7 訂閱落地時必須二選一，並決定 `free` 層還剩多少報價權（要讓需求牆有人回，不能歸零）。
- `[P7] 「平台認證」四個字還沒定稿` — PRD §3.7 讓 `verified` 隨訂閱附帶認證標記，但 COMPLIANCE §2 禁「官方認證」類措辭，首頁又寫著「买得到展示位置，买不到分数」，`docs/BRAND.md` §5.3 也寫明印章永遠不是徽章 — 收費的是**核驗這件事的人工成本**，不是結論；文案上線前必須確定：核驗流程對所有層開放、標記只描述「已提交並通過文件核對」、標記旁掛付費披露、且絕不用印章圖形。
